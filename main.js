function addEventListener_multi(element, eventNames, handler) {
  ...
  function simp_setAlbum(index) {
    simp_cover.innerHTML = simp_a_url[index].dataset.cover ? '<div style="background:url(' + simp_a_url[index].dataset.cover + ') no-repeat;background-size:cover;width:150px;height:150px;"></div>' : '<i class="fa fa-music fa-5x"></i>';
  ...
  if (document.querySelector('#simp')) {
    ...
    var simp_isNext = false;
    var simp_isRandom = false; 
    var simp_isRanext = false; 
    var simp_isStream = false; 
    var simp_isLoaded = false; 
